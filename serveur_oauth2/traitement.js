		...
		// Access token
			function connecterClient(){
					...
					var xhr = $.post("http://localhost/serveur_oauth2/token.php", {scope : "ami",client_id: "testclient", client_secret: "testpass", grant_type: "password", username: username, password: window.localStorage.getItem("password")}, function(data, statusS){
        ...
        function getResources() {
			...
			var jqxhr = $.post("http://localhost/serveur_oauth2/resource.php",{ access_token: window.localStorage.getItem("access_token"), login:  window.localStorage.getItem("user_id")}, function (data, status) {
			alert(data);
			}).done(function () {
			})
			.error(function (resultat, status, error) {alert(error);
		...
		function getResourcesInfo() {
			...
			var jqxhr = $.post("http://localhost/serveur_oauth2/resource2.php",{ access_token: window.localStorage.getItem("access_token"), login:  window.localStorage.getItem("user_id")}, function (data, status) {
			alert(data);
			}).done(function () {
			})
			.error(function (resultat, status, error) {alert(error);
		...
		var jqxhr = $.get("authorize.php",{redirect_uri:"http://localhost/",scope:"ami", client_id:"testclient1", response_type:"code", state:monState}, function (data, status) {
		if (data)
		{
